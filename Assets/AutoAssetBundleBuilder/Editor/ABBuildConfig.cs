...
namespace ReachableGames
{
	namespace AutoBuilder
	{
		...
		// Place an ABBuildConfig in an Editor/Resources folder.  It is really only there to make it easy to configure and generate builds--the asset is not used at runtime.
		...
		public class ABBuildConfig : ScriptableObject
		{
			...
			[Tooltip("You can add fields to the ABBootstrap class if you want.  This is the first file loaded on startup, which you can use to set URLs to server endpoints, for example.")]
			...
			[Tooltip("Any assets whose full path EndsWith any entries here will NOT be added to an asset bundle.  Only use lowercase and forward slashes.  Examples: .pdf or _ignored.asset")]
			public string[] ignoreEndsWith;
			[Tooltip("Any assets whose path that Contains any entries here will NOT be added to an asset bundle.  Only use lowercase and forward slashes.  Examples: /ignore/ or _nobundle")]
			public string[] ignoreContains;
			[Tooltip("Explicit full path of an asset can be added here, but not generally recommended, since you can't easily reorganize files in folders.  Only use lowercase and forward slashes.  Example: assets/bundles/bundlex/filename.png")]
			...
			public class PlatformOptions
			{
				...
				[Tooltip("If enabled, the asset bundles will be embedded in the build in /StreamingAssets/ and the executable will NOT attempt to load anything from the internet.")]
...
//				public bool DeterministicAssetBundle = true;  // This feature is always enabled, according to the docs, so no need to expose it.
				public bool DisableLoadAssetByFileName = true;
				public bool DisableLoadAssetByFileNameWithExtension = true;
	
				// This creates the default options for building the player for all platforms.  Again, if you want something specific for just one platform, do that in code below.
				[Space]
...
//						options |= BuildAssetBundleOptions.DeterministicAssetBundle;
			...
			public Dictionary<string, ABBuildInfo> ConfigBuildTargets(string buildVersion, ABBuildConfig buildConfig)
			{
				...
				foreach (PlatformOptions po in platforms)
				{
					if (po.enabled)
					{
						...
						string playerBuildFolder = ABEditorUtilities.GetPlayerBuildFolder(platformString);
						string configBuildFolder = ABEditorUtilities.GetConfigBuildFolder();  // dest folder for all configuration files, which tend to NOT go to the CDN, but instead to a web site
						string bundleBuildFolder = ABEditorUtilities.GetBundleBuildFolder(platformString);  // dest asset bundles folder  
						string absSrcBundlesFolder = Application.dataPath + buildConfig.sourceBundleFolder;
						string embedBundlesFolder = ABEditorUtilities.GetEmbedBundlesFolder();  // If this is a selfContainedBuild, this is where we will put the config.json, manifest, and bundles just before building the Player.
						string exeName = ABEditorUtilities.CreateExecutableName(po.Platform);  // this cleans up your project name so it can be a valid filename
						...
						// Generate the .json file contents from the basic config.  If this is an embedded build, rewrite the path to the manifest.json so it works out properly.
						ABBootstrap tmpBootstrap = new ABBootstrap(bootstrap);  // had to make a copy here or we would end up changing an asset on disk by accident.
						...
						buildTargets.Add(platformString, new ABBuildInfo(po.Platform, platformString, playerOptions, bundleOptions, 
							playerBuildFolder, exeName, absSrcBundlesFolder, bundleBuildFolder, configBuildFolder, configJson, po.selfContainedBuild, po.WaitForDebuggerConnection, po.ServerBuild,
							embedBundlesFolder, buildVersion, buildConfig.doLogging, buildConfig.ignoreEndsWith, buildConfig.ignoreContains, buildConfig.ignoreExact));
					}
				}
