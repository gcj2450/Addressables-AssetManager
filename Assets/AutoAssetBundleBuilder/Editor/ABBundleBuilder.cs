	...
	namespace AutoBuilder
	{
		...
		public class ABBundleBuilder
		{
			...
			static public bool DoBuildBundles(string srcBundleFolder, string buildVersion, string outputPath, BuildAssetBundleOptions opt, BuildTarget platform, string platformString, bool doLogging, string[] ignoreEndsWith, string[] ignoreContains, string[] ignoreExact)
			{
				...
				{ 
					Debug.LogError("<color=#ff8080>Root of bundles folders does not exist ("+srcBundleFolder+")  Set 'Source Bundle Folder' in ABBuildConfig</color>"); 
					ABMenuItems.RevealBuildConfigs();
				}
			...
			static private List<string> DetermineBundleOrder(Dictionary<string, HashSet<string>> dependencyMatrix, AssetBundleBuild[] buildMap, string assetBundlesFolder)
			{
				...
				while (bundlesRemaining.Count>0)
				{
					...
					if (placedABundle==false)
					{
						...
						{
							// build a list of direct dependencies per asset.  Note, this returns the mixed case filenames of things, and returns ALL dependencies including script files and built-in objects, etc.
							...
							foreach (string dependency in deps)
							{
								...
								// this should never fail because any asset that is listed in a remaining bundle SHOULD only be dependent on nothing or assets in unplaced bundles.
						...
						{
							// This loop actually tracks through the dependency matrix until it arrives at itself, or gives up.  If it doesn't cycle, it's not an error, just a bundle that depends on some that cycle.
				...
				return bundlesInMinimumDependencyOrder;  // this returns the exact order that asset bundles need to be loaded and mounted for assets to resolve.
			}
															...
															List<string> bundleOrder, AssetBundleBuild[] buildMap, AssetBundleManifest unityManifest)
			{
				...
				foreach (string bundle in bundleOrder)  // bundle is a short name (foo.assetbundle)
				{
					...
					if (buildMapIndex==-1) Debug.LogError("<color=#ff8080>Asset bundle "+bundle+" was not found in the buildMap, so the asset list would be empty!</color>");
					...
					ABManifest.ABAssetBundleInfo bundleInfo = new ABManifest.ABAssetBundleInfo(bundleWithoutSuffix, hash128, flength, bundleIndices.ToArray(), 
																								buildMap[buildMapIndex].assetNames, assetIndices, assetHashes);
			...
			static public void DoOverrideBundleBuild(ABBuildInfo buildInfo, ABBuild existingBuild)
			{
				...
				string manifestFullPath = ABUtilities.GetRuntimeCacheFolder() + "/manifest-" +existingBuild.buildVersion+".json";
				...
				{
					// Create an asset bundle for each folder containing all the child assets inside it.
					string bundleName = "override-"+existingBuild.buildVersion+".assetbundle";
					...
					if (unityManifest==null) Debug.LogError("<color=#ff8080>["+existingBuild.platform+"] Asset bundle build failed for "+buildMap[0].assetBundleName+"</color>");
				...
				string manifestPath = ABUtilities.GetRuntimeCacheFolder() + "/override-"+existingBuild.buildVersion+".json";
				File.WriteAllText(manifestPath, manifestJson, System.Text.Encoding.UTF8);
			}
