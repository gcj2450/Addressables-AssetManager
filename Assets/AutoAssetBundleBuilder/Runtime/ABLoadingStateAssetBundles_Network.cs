...
namespace ZionGame
{
    ...
    public class ABLoadingStateAssetBundles_Network : ABILoadingState
    {
        ...
        string errorStr = "";
        float progress = 0;
        public ABLoadingStateAssetBundles_Network(string bundleVersion, string bundleFolderPrefix, EventHandler eventHandler)
        {
        ...
        public float GetProgress()
        {
            ...
            //Debug.Log($"_lastProgress {_lastProgress}, _assetIndex:{_assetIndex}, _manifest.assetBundles.Length:{_manifest.assetBundles.Length} ");
        ...
        private void StartNextBundle(int index)
        {
            ...
            if (_assetIndex >= _manifest.assetBundles.Length)
            {
                ...
                //    _request.Dispose();
                //_request = null;
                Debug.Log("_assetIndex >= _manifest.assetBundles.Length");
            }
            else
            {
        ...
        public void LoomDownload(string url, string savePath, Action<float> onProgreeUpdate, Action<bool, string> onComplete)
        {
            ...
            Loom.RunAsync(() =>
            {
                ...
                //{
                //    //Set the vertices
                //    //_resObj.downloadMark.enabled = false;
                //   // _resObj.isDownloaded = true;
                //    //PlayerPrefs.DeleteKey(_resObj.myVideoRes.title);
                //    //StaticData.Instance.AlreadyDownloadedDatas.Add(_resObj.myVideoRes);
                //});

            });
        }

        public void ThreadDownload(string url, string savePath, Action<float> onProgreeUpdate, Action<bool, string> onComplete)
        {
            ThreadStart starter = delegate { simpleDownload(url, savePath, onProgreeUpdate, onComplete); };
        ...
        public void simpleDownload(string url, string savePath, Action<float> onProgreeUpdate, Action<bool, string> onComplete)
        {
            ...
            else
            {
                ...
                //else
                //{
                //    //文件不存在，这个情况就是writer创建或者打开文件失败了，上面已经考虑了
                //    try
                //    {
                //        response = request.GetResponse();
                //    }
                //    catch (Exception ex)
                //    {
                //        Debug.Log("response = request.GetResponse() error: "+ex.Message);
                //        if (onComplete != null)
                //        {
                //            onComplete(false, ex.Message);
                //        }
        ...
        private void BundleLoaded(AsyncOperation asyncOp)
        {
            ...
            catch (Exception e)
            {
                if (OnError != null)
                {
                    Debug.LogError("Failed to open bundle " + e.Message);
