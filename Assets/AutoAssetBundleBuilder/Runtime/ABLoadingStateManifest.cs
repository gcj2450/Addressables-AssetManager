...
namespace ReachableGames
{
	namespace AutoBuilder
	{
		public class ABLoadingStateManifest : ABILoadingState
		{
			...
			public ABLoadingStateManifest(string overrideManifestPath, string bundleFolderPrefix)
			{
			...
			public void Begin(Dictionary<string, ABIData> configData)
			{
				...
				_manifestFullPath = ABUtilities.GetRuntimeCacheFolder() + "/" + _manifestFilename;

				// If we already have this version of the manifest cached, just jump to the completion callback, since it's going to load it from disk anyway.
...
// But if it were needed, this would be where that code would go.
			...
			{
				string path = ABUtilities.GetRuntimeCacheFolder();
