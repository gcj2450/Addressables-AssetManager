namespace CoreFtp
{
    ...
    public class FtpClient : IFtpClient
    {
        ...
        public async Task LogOutAsync()
        {
            ...
            Debug.Log("[FtpClient] Logging out");
        ...
        {
            Debug.Log($"[FtpClient] changing directory to {directory}");
        ...
        public async Task CreateDirectoryAsync(string directory)
        {
            ...
            Debug.Log($"[FtpClient] Creating directory {directory}");
        ...
        {
            EnsureLoggedIn();
            Debug.Log($"[FtpClient] Renaming from {from}, to {to}");
        ...
        public async Task DeleteDirectoryAsync(string directory)
        {
            ...
            Debug.Log($"[FtpClient] Deleting directory {directory}");
        ...
        {
            EnsureLoggedIn();
            Debug.Log($"[FtpClient] Setting client name to {clientName}");
        ...
        {
            Debug.Log($"[FtpClient] Opening file read stream for {fileName}");
        ...
        public async Task<Stream> OpenFileWriteStreamAsync(string fileName)
        {
            ...
            Debug.Log($"[FtpClient] Opening file read stream for {filePath}");
        ...
        {
            Debug.Log("[FtpClient] Closing write file stream");
            dataStream.Dispose();
        ...
        public async Task<ReadOnlyCollection<FtpNodeInformation>> ListAllAsync()
        {
            ...
            {
                EnsureLoggedIn();
                Debug.Log($"[FtpClient] Listing files in {WorkingDirectory}");
        ...
        public async Task<ReadOnlyCollection<FtpNodeInformation>> ListDirectoriesAsync()
        {
            ...
            {
                EnsureLoggedIn();
                Debug.Log($"[FtpClient] Listing directories in {WorkingDirectory}");
        ...
        {
            EnsureLoggedIn();
            Debug.Log($"[FtpClient] Deleting file {fileName}");
            var response = await ControlStream.SendCommandAsync(new FtpCommandEnvelope
            {
        ...
        {
            EnsureLoggedIn();
            Debug.Log($"[FtpClient] Setting transfer mode {transferMode}, {secondType}");
            var response = await ControlStream.SendCommandAsync(new FtpCommandEnvelope
            {
        ...
        {
            EnsureLoggedIn();
            Debug.Log($"[FtpClient] Getting file size for {fileName}");
        ...
        {
            Debug.Log("[FtpClient] Determining directory provider");
            if (this.UsesMlsd())
        ...
        {
            EnsureLoggedIn();
            Debug.Log("[FtpClient] Determining features");
        ...
        {
            Debug.Log($"[FtpClient] Creating directory structure recursively {string.Join("/", directories)}");
        ...
        {
            EnsureLoggedIn();
            Debug.Log($"[FtpClient] Opening filestream for {fileName}, {command}");
        ...
        {
            Debug.Log("[FtpClient] Connecting to a data socket");
        ...
        private async Task BailIfResponseNotAsync(FtpResponse response, params FtpStatusCode[] codes)
        {
            ...
            Debug.Log($"Bailing due to response codes being {response.FtpStatusCode}, which is not one of: [{string.Join(",", codes)}]");
        ...
        private async Task IgnoreStaleData()
        {
            if (IsConnected && ControlStream.SocketDataAvailable())
            {
                ...
                Debug.Log($"Stale data detected: {staleData.ResponseMessage}");
            }
        }

        public void Dispose()
        {
            Debug.Log("Disposing of FtpClient");
