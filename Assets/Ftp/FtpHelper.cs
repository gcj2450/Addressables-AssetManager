...
namespace UnityFtp
{
    ...
    public partial class FtpHelper : IDisposable
    {
        ...
        public bool Connect()
        {
            ...
            catch (Exception ex) { Debug.LogError(ex.Message); this._ErrorMsg = ex.ToString(); }
            return Success;
        }

        public void SetPassiveMode(bool isPassive)
        {
            if (isPassive)
                this._FtpConnect.SendCommand("PASV");//被动模式传输
            else
                this._FtpConnect.SendCommand("PORT");//主动模式传输
        ...
        public long GetFileSize(string remoteFileName)
        {
            ...
            catch (Exception ex) { Debug.LogError(ex.Message); throw ex; }
            return lFileSize;
        }
        #region 删除指定的文件
        /// <summary>
        ...
        private long readFileSize(FTPConnect ftpCon, FtpFileInfo ftpFileInfo)
        {
            ...
            {
                //切换目录
                ftpCon.TransformDir(ftpFileInfo.DirectoryName);
        ...
        {
            return this.UploadFileAsync(filePath, ftpFileName, false, true);
        }
        /// <summary>
        /// 上传文件
        /// </summary>
        ...
        public string UploadFileAsync(string filePath, string ftpFileName, bool isContinueUpload, bool deleteExist)
        {
            string strFTPId = System.Guid.NewGuid().ToString();

            //如果文件已存在，并且是删除存在的情况下，删除存在的文件
            if (FileExist(ftpFileName) && deleteExist)
            {
                Debug.Log("File Exists: " + ftpFileName + ", will be deleted");
                DeleteFile(ftpFileName);
            }

            string remoteFileName = this.FtpUrl.Path + "/" + ftpFileName;
            FtpFileInfo ftpFileInfo = new FtpFileInfo(remoteFileName);
            IList<object> objList = new List<object> { filePath, ftpFileInfo, isContinueUpload, strFTPId };
