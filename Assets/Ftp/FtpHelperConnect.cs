    ...
    partial class FtpHelper
    {
        ...
        public class FTPUrl : ICloneable
        {
            ...
            public FTPUrl(Uri uri, string userName, string userPassword)
            {
                ...
                catch (Exception ex) { Debug.LogError(ex.Message); m_RemoteHostIP = IPAddress.Parse("127.0.0.1"); }
        ...
        public class FtpFileInfo
        {
            ...
            private string getDirectoryName()
            {
                if (_fullName.IndexOf('/') >= 0)
                {
                    //路径中即使包含'/'符号，也不能保证最后一项是文件，但假设最后一个是文件，
                    //而取得从第一个到倒数第二个，且最后一个目录不包含"/"符号
